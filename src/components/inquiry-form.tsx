...
export function InquiryForm({ propertyId, propertyTitle }: { propertyId: string; propertyTitle: string }) {
  ...
  return (
    <form onSubmit={submit} className="p-6 bg-card border border-border rounded-sm space-y-3">
      ...
      <input className="w-full h-10 px-3 border border-border rounded-sm text-sm bg-background" placeholder="Presupuesto (ej. Q1,500,000 o USD 200,000)" value={form.budget} onChange={(e) => setForm({ ...form, budget: e.target.value })} maxLength={60} />
