...
export function ConversationsInbox() {
  ...
  /*
   * Reloj para mantener actualizada
   * la ventana de WhatsApp de 24 horas.
   */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error || !data?.organization_id) {
        toast.error(
          "No fue posible identificar la organización del usuario.",
        );
  ...
  const loadInbox = useCallback(
    async (silent = false) => {
      ...
      const {
        ...
        .from("wa_messages")
        .select(
          "id,organization_id,lead_id,phone,role,content,sender,message_type,media_url,created_at",
        )
      ...
      if (messagesError) {
        toast.error(
          "Conversaciones: " + messagesError.message,
        );
      ...
      const [
        ...
        db
          .from("leads")
          .select(
            "id,organization_id,full_name,phone,email,source,interest_operation,interest_type,interest_zone,budget_min,budget_max,currency,bedrooms,bathrooms,notes,temperature,status,property_id,ad_property_id,assigned_to_user_id,created_by,ai_paused,updated_at",
          )
          .eq("organization_id", organizationId)
          .in("id", leadIds),
      ...
      if (readsError) {
        console.error(
          "Error cargando estados de lectura:",
          readsError,
        );
      }
      ...
      const [
        ...
        propertyIds.length
          ? db
              .from("properties")
              .select(
                "id,title,zone,price,currency,operation,cover_image",
              )
              .in("id", propertyIds)
          : Promise.resolve({
              data: [],
        ...
        visibleLeadIds.length
          ? db
              .from("deals")
              .select(
                "lead_id,created_at,deal_stages(name,slug)",
              )
       ...
       */
      const stageMap =
        new Map<string, StageRow>();

      (
        (dealsResult.data ?? []) as any[]
      ).forEach((deal) => {
        if (
          !deal.lead_id ||
          stageMap.has(deal.lead_id)
        ) {
          return;
        }
      ...
      /*
       * Última lectura por conversación.
       */
      const readMap =
      ...
      const next = leads
        .map((lead) => {
          ...
          /*
           * Solo cuentan como no leídos
           * los mensajes del CLIENTE
           * posteriores a last_read_at.
           */
          const unreadCount =
            thread.filter(
              (message) =>
      ...
      /*
       * IMPORTANTE:
       * ya NO seleccionamos automáticamente
       * la primera conversación.
       *
       * Solo permanece seleccionada
       * si el usuario ya la tenía abierta.
       */
      setSelectedLeadId(
        (current) =>
          current &&
  ...
  /*
   * Registrar conversación como leída.
   *
   * No usamos upsert directamente
   * porque queremos detectar cualquier
   * problema de escritura con claridad.
   */
  const markConversationAsRead =
    useCallback(
      async (leadId: string) => {
        ...
        /*
         * Cambio optimista:
         * quitar contador inmediatamente.
         */
        setConversations(
          (rows) =>
            rows.map((row) =>
        ...
        if (findError) {
          console.error(
            "Error buscando estado de lectura:",
            findError,
          );
        ...
        if (existing) {
          ...
          if (updateError) {
            console.error(
              "Error actualizando conversación leída:",
              updateError,
            );
          }
        ...
        if (insertError) {
          console.error(
            "Error registrando conversación leída:",
            insertError,
          );
        }
      },
   ...
   * 1. Nuevos mensajes.
   * 2. Cambios en leads.
   * 3. Cambios en estados de lectura.
   */
  useEffect(() => {
    if (!organizationId) return;
   ...
   *
   * También se vuelve a marcar si llega
   * un mensaje nuevo mientras el chat
   * permanece abierto.
   */
  useEffect(() => {
    if (
  ...
  async function sendText() {
    ...
    if (!windowOpen) {
      return toast.error(
        "La ventana de 24 horas está cerrada. Debe usarse una plantilla aprobada.",
      );
    }
    ...
    toast.success(
      "Mensaje enviado. El chat queda en control humano.",
    );
  ...
  async function sendPhotos() {
    ...
    if (
      ...
      return toast.error(
        "Este lead no tiene una propiedad enlazada con fotos.",
      );
    }
    ...
    toast.success(
      `Fotos enviadas: ${data?.sent ?? 0}. El chat queda en control humano.`,
    );
  ...
  async function toggleAi() {
    ...
    toast.success(
      nextPaused
        ? "Control humano activado."
        : "Conversación devuelta a Andrea.",
    );
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Andrea filtra y califica;
            tú puedes entrar al chat y
            tomar el cierre cuando haga
            falta.
          </p>
        </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                            <span
                              className={`text-[10px] px-1.5 py-0.5 border rounded-full ${tempClasses(
                                conversation
                                  .lead
                                  .temperature,
                              )}`}
          ...
          {!selectedConversation ? (
            ...
            <>
              <div className="px-4 py-3 border-b border-border bg-card flex items-center justify-between gap-3">
                ...
                <div
                  className={`inline-flex items-center gap-1.5 text-[11px] px-2 py-1 rounded-full border shrink-0 ${
                    windowOpen
                ...
                {selectedConversation.messages.map(
                  (message) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-[88%] sm:max-w-[75%] rounded-xl px-3 py-2 shadow-sm ${bubbleClass}`}
                        >
                          <p
                          ...
                          {message.message_type ===
                            ...
                            message.media_url && (
                              <img
                                ...
                                className="rounded-lg max-h-72 w-auto object-cover mb-2"
                                loading="lazy"
                              />
                            )}
              ...
              <div className="border-t border-border bg-card p-3">
                {!windowOpen && (
                  <div className="mb-2 px-3 py-2 rounded-sm border border-amber-200 bg-amber-50 text-xs text-amber-900">
                    Meta no permite texto
                    libre desde este inbox
                    hasta que el cliente
                    escriba nuevamente o se
                    use una plantilla
                    aprobada.
                  </div>
                )}
                ...
                <div className="flex items-end gap-2">
                  <textarea
                    ...
                    placeholder={
                      windowOpen
                        ? "Escribe como asesor…"
                        : "Ventana cerrada: usa una plantilla aprobada"
                    }
            ...
            <div className="p-4 space-y-5">
              <div>
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <span
                    className={`text-[11px] px-2 py-0.5 border rounded-full ${tempClasses(
                      selectedConversation
                        .lead
                        .temperature,
                    )}`}
                  ...
                  <span className="text-[11px] px-2 py-0.5 border border-border rounded-full">
                    {selectedConversation
                      .stage?.name ??
                      selectedConversation
                        .lead.status ??
              ...
              <div className="space-y-2">
                <button
                  ...
                  disabled={togglingAi}
                  className={`w-full h-10 px-3 rounded-sm text-sm font-semibold inline-flex items-center justify-center gap-2 border ${
                    selectedConversation
                      .lead.ai_paused
                ...
                <Link
                  ...
                  className="w-full h-10 px-3 rounded-sm border border-border bg-card text-sm font-semibold inline-flex items-center justify-center gap-2 hover:bg-muted"
                >
                  <CalendarDays
                    size={15}
                  />
              ...
              >
                ...
                <p className="mt-1 opacity-80">
                  Al enviar desde este
                  inbox, el lead se marca
                  automáticamente en
                  control humano.
                </p>
              </div>
