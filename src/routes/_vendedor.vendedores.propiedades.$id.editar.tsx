    ...
    <div className="container-altum py-10">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[1.6fr_1fr]">
            ...
            <div className="mt-3 grid grid-cols-4 gap-3">
              {images.slice(0, 8).map((src, i) => (
                <button 
                  ...
                  className={"aspect-[4/3] overflow-hidden rounded-sm border " + (i === activeImg ? "border-secondary" : "border-border opacity-80")}
                >
        ...
        <aside className="space-y-6">
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-secondary font-semibold">{p.type} · {p.operation === "venta" ? "En venta" : "En renta"}</p>
            <h1 className="font-display text-3xl md:text-4xl text-primary mt-2">{p.title}</h1>
            <p className="mt-2 flex items-center gap-1 text-sm text-muted-foreground">
              <MapPin size={14} /> {p.zone}
            </p>
            <p className="mt-4 text-4xl price-text">
              {formatGTQ(p.price)}
